    ...
    methods (Static)
     ...
     methods (Static)
        ...
        function J=back_propagate(inputs,nn,targets,learning_rate,batch)
            ...
            sum_over_outputs = zeros(1,length(nn.hidden_neurons));
            ...
            for j=1:length(nn.hidden_neurons)
                for i=1:length(outputs)
                    sum_over_outputs(j) = sum_over_outputs(j) + nn.output_weights(j,i) * output_deltas(i);
                end
                hidden_deltas(j) = sigmoid_derivative(nn.hidden_neurons(j)) * sum_over_outputs(j);
            end

            % Step 3. update weights output --> hidden
            for j=1:length(nn.hidden_neurons)
                for i=1:length(output_deltas)
                    nn.output_weights(j,i) =nn.output_weights(j,i) -(output_deltas(i) * nn.hidden_neurons(j) * learning_rate);
                end
            end
